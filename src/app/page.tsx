...
export default function HomePage() {
  ...
  const visibleGroups = selectedSport === "all" ? groups.slice(0, 3) : groups.filter((group) => group.sport === selectedSport);

  return (
    <main className="min-h-[100dvh] overflow-hidden bg-[var(--paper)] text-[var(--ink)]">
      <nav className="mx-auto flex max-w-[1400px] items-center justify-between px-5 py-5 sm:px-8 lg:px-12">
        ...
        <div className="hidden items-center gap-8 text-sm font-semibold text-[var(--muted)] lg:flex">
          {navItems.map((item) => (
            <a className="transition-colors hover:text-[var(--ink)]" href={item.href} key={item.label}>
              {item.label}
            </a>
          ))}
        </div>
        <div className="flex items-center gap-3 text-sm font-bold">
          <AccountControl />
          <Link className="rounded-full bg-[var(--accent)] px-4 py-2.5 text-[var(--accent-foreground)] transition-transform hover:-translate-y-0.5 active:translate-y-px" href="/sign-up">Start a group</Link>
        </div>
      </nav>

      <section className="mx-auto grid max-w-[1400px] items-center gap-10 px-5 pb-16 pt-8 sm:px-8 sm:pb-20 lg:grid-cols-[0.9fr_1.1fr] lg:gap-16 lg:px-12 lg:pb-24 lg:pt-12">
        ...
        <div className="relative min-h-[430px] sm:min-h-[520px] lg:min-h-[570px]">
          ...
          <div className="relative h-[430px] overflow-hidden rounded-[2rem] bg-[#d9c7ad] shadow-[0_22px_60px_rgba(37,53,44,0.15)] sm:h-[520px] lg:h-[570px]">
            <Image alt="Women playing floorball together in a bright indoor sports hall" className="h-full w-full object-cover object-[62%_center]" fill priority sizes="(max-width: 1024px) 100vw, 56vw" src="/images/sports-community-hero.png" />
            <div className="absolute inset-0 bg-gradient-to-t from-[#b74b2f]/65 via-transparent to-transparent" aria-hidden="true" />
            <div className="absolute bottom-5 left-5 right-5 flex items-end justify-between gap-4 text-white sm:bottom-7 sm:left-7 sm:right-7">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.14em] text-white/75">Tonight in Stockholm</p>
                <p className="mt-2 text-2xl font-extrabold tracking-[-0.04em]">Söder Sparks</p>
                <p className="mt-1 text-sm text-white/80">12 people are going</p>
              </div>
      ...
      <section className="border-y border-[var(--line)] bg-[var(--surface-muted)]" id="sports">
        <div className="mx-auto max-w-[1400px] px-5 py-16 sm:px-8 lg:px-12 lg:py-20">
          ...
          <div className="mt-10 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
            ...
            {sports.map((sport) => (
              <button aria-pressed={selectedSport === sport.slug} className={`group min-h-36 rounded-[1.25rem] border-2 p-4 text-left transition-all hover:-translate-y-1 hover:outline hover:outline-2 hover:outline-offset-2 hover:outline-[var(--ink)] ${selectedSport === sport.slug ? "border-[var(--ink)]" : "border-transparent"}`} key={sport.slug} onClick={() => setSelectedSport(sport.slug)} style={{ backgroundColor: sport.accent, color: sport.color }} type="button">
                <span className="block text-2xl font-black tracking-[-0.07em] transition-transform group-hover:translate-x-1">{sport.name}</span>
                <span className="mt-8 block text-xs font-bold text-[var(--ink)]">{sport.count}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      <section className="bg-[var(--paper)]" id="discover">
        ...
        <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          ...
          <button className="w-fit rounded-full border border-[var(--line-strong)] px-4 py-2.5 text-sm font-bold transition-colors hover:border-[var(--ink)]" type="button">Stockholm, Sweden</button>
        </div>
        ...
        {visibleGroups.length === 0 && (
          <div className="mt-10 rounded-[1.5rem] border border-dashed border-[var(--line-strong)] bg-[var(--surface)] px-6 py-14 text-center">
            ...
            <Link className="mt-6 inline-block rounded-full bg-[var(--accent)] px-5 py-3 text-sm font-extrabold text-[var(--accent-foreground)]" href="/sign-up">Start a group</Link>
          </div>
        )}
        </div>
      </section>

      <section className="mx-5 mb-6 overflow-hidden rounded-[1.75rem] border border-[var(--line)] bg-[var(--surface-muted)] text-[var(--ink)] sm:mx-8 lg:mx-12" id="for-organizers">
        <div className="mx-auto grid max-w-[1400px] items-center gap-8 px-6 py-12 sm:px-10 lg:grid-cols-[1fr_auto] lg:px-14 lg:py-16">
          <div>
          ...
          <Link className="w-fit rounded-full bg-[var(--accent)] px-6 py-3.5 text-sm font-extrabold text-[var(--accent-foreground)] transition-transform hover:-translate-y-1" href="/sign-up">Start a group</Link>
        </div>
      </section>

      <footer className="mx-auto flex max-w-[1400px] flex-col gap-3 px-5 py-8 text-sm text-[var(--muted)] sm:flex-row sm:items-center sm:justify-between sm:px-8 lg:px-12">
        <p className="font-display text-lg font-black tracking-[-0.06em] text-[var(--ink)]">huddle<span className="text-[var(--accent-strong)]">.</span></p>
